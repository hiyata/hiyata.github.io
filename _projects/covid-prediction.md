...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateMetrics(data) {
        ...
        document.getElementById('lstm-gru-mape').textContent = fmt(mape.lstm_gru);
        document.getElementById('arima-mape').textContent = fmt(mape.arima);
        document.getElementById('rf-mape').textContent = fmt(mape.random_forest);
        document.getElementById('xgb-mape').textContent = fmt(mape.xgboost);
        document.getElementById('last-updated').textContent = data.last_updated
            ? dayjs(data.last_updated).format('MMMM D, YYYY HH:mm:ss')
            : (data.dates ? data.dates[data.dates.length - 1] : 'N/A');
    }
    ...
    function createComparisonChart(data) {
        ...
        const traces = [actualTrace, lstmGruTrace, arimaTrace, rfTrace];

        if (data.xgb_predictions) {
            traces.push({
                x: data.dates,
    ...
    if (typeof Plotly === 'undefined') {
        ...
        plotlyScript.src = 'https://cdnjs.cloudflare.com/ajax/libs/plotly.js/3.4.0/plotly.min.js';
